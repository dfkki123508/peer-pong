...
export default function sketch(p) {
  ...
  class Game {
    ...
    constructor() {
      ...
      this.player1StartingPosition = p.createVector(10, window.innerHeight / 2);
      this.player2StartingPosition = p.createVector(window.innerWidth - 10, window.innerHeight / 2);
      this.playerMoveSpeed = 5 * window.innerHeight / 500;  // empirical value
      this.score = [0, 0];
      this.resetRound();
    }
    ...
    drawScore() {
      ...
      p.fill(255);
      let gap = 40;
      p.text(this.score[0], window.innerWidth / 2 - gap, 40);
      p.text(this.score[1], window.innerWidth / 2 + gap, 40);
    }
    ...
    movePlayers() {
      ...
      } else if (p.keyIsDown(p.DOWN_ARROW) && this.player2.position.y + this.player2.size.height / 2 < window.innerHeight - this.borderWidth) {
        this.player2.position.y += this.playerMoveSpeed;
      }
      ...
      } else if (p.keyIsDown(83) && this.player1.position.y + this.player1.size.height / 2 < window.innerHeight - this.borderWidth) {
        this.player1.position.y += this.playerMoveSpeed;
      }
    }
    ...
    detectCollision() {
      ...
      else if (this.ball.position.x + this.ball.radius / 2 > window.innerWidth - this.borderWidth || this.ball.position.x - this.ball.radius / 2 < this.borderWidth) { // left/right, out of game
        console.log('LEFT/RIGHT');
        let scoreIdx = +!(this.ball.position.x + this.ball.radius / 2 > window.innerWidth - this.borderWidth)
        this.score[scoreIdx]++;
        this.resetRound(false);
      } else if (this.ball.position.y + this.ball.radius / 2 > window.innerHeight - this.borderWidth || this.ball.position.y - this.ball.radius / 2 < this.borderWidth) {  // top/bottom
    ...
    resetRound(start = true) {
      ...
      this.ball = new Ball(p.createVector(window.innerWidth / 2, window.innerHeight / 2), ballStartingVelocity, ballStartingAccelartion, 20, this.maxBallSpeed);
    }
  }
  ...
  class Background {
    ...
    draw() {
      for (let i = 0; i < window.innerHeight; i += this.dashDist) {
        p.line(window.innerWidth / 2, i, window.innerWidth / 2, i + this.dashDist / 2);
      }
    }
